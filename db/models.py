...
class Ticket(models.Model):
    ...
    def clean(self) -> None:
        errors = {}
        last_row = self.movie_session.cinema_hall.rows
        last_seat = self.movie_session.cinema_hall.seats_in_row

        if self.row < 1 or self.row > last_row:
            errors["row"] = (f"row number must be in available range: "
                             f"(1, rows): (1, {last_row})")

        if self.seat < 1 or self.seat > last_seat:
            errors["seat"] = (f"seat number must be in available range: "
                              f"(1, seats_in_row): (1, {last_seat})")
